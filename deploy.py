...
GIT_REPO = [
    # [别名,   分支名,     Git Repo 路径]
    ['origin', 'master', 'git@github.com:catchance/catchance.github.io.git'],
]
# 部署到哪里，相对上一级目录。比如下面的配置，会部署到 ../catchance.github.io 目录里
DEPLOY_DIR = 'catchance.github.io'
...
def deploy(args):
    ...
    with ChDir(current_dir):
        ...
        # git log --pretty=format:"%s" => %s - 提交说明
        ...
        # step2 build
        subprocess.call('hugo -v --cacheDir="{0}"'.format(cache_dir), shell=True)
